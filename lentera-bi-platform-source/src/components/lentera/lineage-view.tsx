...
import { AlertTriangle, Database, GitBranch, BarChart3, LayoutDashboard, LineChart, AlertCircle, ChevronRight, ZoomIn, ZoomOut, Maximize } from 'lucide-react';
...
function CustomNode({ data }: NodeProps) {
  ...
  return (
    <div className={`px-3 py-2 rounded-lg border-2 ${config.bg} ${config.border} shadow-sm min-w-[140px] max-w-[200px]`}>
      ...
      <div className="flex items-center gap-1.5 mb-0.5">
        ...
        {data.findings > 0 && (
          <span className="ml-auto flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[9px] text-white font-bold">
            {data.findings}
          </span>
        )}
      </div>
      ...
      {data.platform && (
        <p className="text-[9px] text-muted-foreground mt-0.5">{data.platform}</p>
      )}
...
function layoutNodes(nodes: LineageData['nodes'], edges: LineageData['edges']): { nodes: Node[], edges: Edge[] } {
  ...
  const layerOrder = ['source', 'column', 'model', 'dataset', 'metric', 'filter', 'dimension', 'time_dimension', 'chart', 'dashboard'];
  ...
  activeLayers.forEach((layer, layerIdx) => {
    ...
    layerNodes.forEach((n, nodeIdx) => {
      const y = nodeIdx * ySpacing + 50;
      positionMap[n.id] = { x, y };
    });
  });
...
export function LineageView({ data }: { data: LineageData }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 text-xs">
        {Object.entries(typeConfig).filter(([k]) => types.includes(k)).map(([type, config]) => (
          ...
          >
            ...
            <MiniMap
              ...
              nodeColor={(n) => {
                ...
                return config ? config.border.replace('border-', '') : '#999';
              }}
            />
          </ReactFlow>
        </div>
